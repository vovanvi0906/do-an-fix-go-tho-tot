...
export class AdminService {
   ...
   */
  async getServiceCategories(query = {}) {
    ...
    // 2. Tìm kiếm theo từ khóa (tên, mô tả, hoặc slug)
   ...
   * @param {import('./dto/create-service-category.dto').CreateServiceCategoryDto} data
   */
  async createServiceCategory(data) {
    ...
    let slug = data.slug ? data.slug.trim().toLowerCase() : null;
    if (!slug) {
      slug = name
        .toLowerCase()
        .normalize('NFD')
    ...
    const iconUrl = data.iconUrl?.trim() || data.icon?.trim() || null;
    const icon = data.icon?.trim() || data.iconUrl?.trim() || null;
    ...
    return {
      success: true,
      message: `Tạo mới danh mục "${newCategory.name}" thành công`,
   ...
   * @param {import('./dto/update-service-category.dto').UpdateServiceCategoryDto} data
   */
  async updateServiceCategory(id, data) {
    ...
    return {
      success: true,
      message: `Cập nhật danh mục "${updated.name}" thành công`,
      category: updated,
    };
  }
  ...
  async toggleCategoryStatus(id) {
    ...
    return {
      success: true,
      message: `Đã ${updated.isActive ? 'kích hoạt' : 'tạm ngưng'} danh mục "${updated.name}"`,
      category: updated,
    };
  }

  async deleteServiceCategory(id, force = false) {
    ...
    const hasServices = (existing._count?.services || 0) > 0;

    // 1. Force Delete: Xóa triệt để các dịch vụ trực thuộc rồi xóa danh mục
    if (isForce) {
      if (hasServices) {
        ...
        for (const s of services) {
          await this.deleteService(s.id, true);
        }
      }
      ...
      return {
        success: true,
        message: `Đã xóa vĩnh viễn danh mục "${existing.name}" cùng các dịch vụ liên kết khỏi hệ thống.`,
        id,
        permanentlyDeleted: true,
      };
    }

    // 2. Nếu có dịch vụ liên kết -> Chuyển sang Tạm ngưng (Soft Delete)
    if (hasServices) {
      const updated = await this.prisma.serviceCategory.update({
        where: { id },
        data: { isActive: false },
      ...
      return {
        success: true,
        message: `Danh mục "${existing.name}" đang có ${existing._count.services} dịch vụ liên kết. Đã chuyển sang trạng thái tạm ngưng. Bật "Xóa vĩnh viễn" nếu muốn xóa bỏ hoàn toàn.`,
        category: updated,
        softDeleted: true,
        hasServices: true,
      };
    ...
    // 3. Nếu không có dịch vụ -> Xóa vĩnh viễn bình thường
    await this.prisma.serviceCategory.delete({
      where: { id },
    });
  ...
  async deleteService(id, force = false) {
    ...
    const hasOrders = (existing._count?.orders || 0) > 0;
    const hasWorkers = (existing._count?.workerServices || 0) > 0;

    // 1. Trường hợp Admin yêu cầu xóa vĩnh viễn (Force Delete)
    if (isForce) {
      if (hasOrders) {
        ...
        if (orderIds.length > 0) {
          ...
          await this.prisma.orderImage.deleteMany({
            where: { orderId: { in: orderIds } },
          });
      ...
      return {
        success: true,
        message: `Đã xóa vĩnh viễn dịch vụ "${existing.name}" khỏi cơ sở dữ liệu.`,
        id,
        permanentlyDeleted: true,
      };
    }

    // 2. Nếu có dữ liệu liên kết mà không bật force -> Chuyển sang Tạm ngưng (Soft delete)
    if (hasOrders || hasWorkers) {
      const updated = await this.prisma.service.update({
        where: { id },
        data: { isActive: false },
      ...
      return {
        success: true,
        message: `Dịch vụ "${existing.name}" đã có dữ liệu liên kết (${existing._count?.orders || 0} đơn). Đã chuyển sang trạng thái tạm ngưng. Bật "Xóa vĩnh viễn" nếu muốn xóa bỏ hoàn toàn.`,
        service: updated,
        softDeleted: true,
        hasOrders,
      };
    ...
    // 3. Nếu không có dữ liệu liên kết -> Xóa vĩnh viễn bình thường
    await this.prisma.service.delete({ where: { id } });
